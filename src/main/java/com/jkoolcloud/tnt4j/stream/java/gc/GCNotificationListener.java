...
/**
 * This class provides GC notification listener that extracts relevant GC metrics and streams over TNT4J framework. GC
 * JMX notifications available in JDK1.7.04 or higher.
 * 
 * @version $Revision: 1 $
 * 
 */
public class GCNotificationListener implements NotificationListener {
	...
	/**
	 * Create GC notification listener with a given {@link com.jkoolcloud.tnt4j.TrackingLogger} instance and resource
	 * name.
	 * 
	 * @param lg
	 *            {@code TrackingLogger} instance where all GC activities are logged
	 * @param resourceName
	...
	public void handleNotification(Notification notification, Object handback) {
		if (notification.getType().equals(GarbageCollectionNotificationInfo.GARBAGE_COLLECTION_NOTIFICATION)) {
			...
			TrackingEvent gcEvent = logger.newEvent(OpLevel.TRACE, OpType.CLEAR, info.getGcAction(), null,
					info.getGcName(), msg, info.getGcAction(), info.getGcInfo().getId(), notification.getType(),
					notification.getMessage(), info.getGcInfo().getDuration(), totalGcDuration, gcPercent,
					info.getGcCause(), info.getGcInfo().getStartTime(), info.getGcInfo().getEndTime());
			...
			for (Entry<String, MemoryUsage> entry : mem.entrySet()) {
				...
				memoryAfter.add("memUsedDelta", (memAfter.getUsed() - memBefore.getUsed()),
			...
			gcEvent.getOperation()
					.addProperty(logger.newProperty("gcId", info.getGcInfo().getId(), ValueTypes.VALUE_TYPE_COUNTER));
			gcEvent.getOperation()
					.addProperty(logger.newProperty("gcCycleUsage", gcUsage, ValueTypes.VALUE_TYPE_PERCENT));
			gcEvent.getOperation().addProperty(logger.newProperty("gcCycleDuration", info.getGcInfo().getDuration(),
					ValueTypes.VALUE_TYPE_AGE_MSEC));
			gcEvent.getOperation().addProperty(
					logger.newProperty("totalGCDuration", totalGcDuration, ValueTypes.VALUE_TYPE_AGE_MSEC));
			gcEvent.getOperation()
					.addProperty(logger.newProperty("totalGCOverhead", gcPercent, ValueTypes.VALUE_TYPE_PERCENT));
			logger.tnt(gcEvent);
		}
	}
}
